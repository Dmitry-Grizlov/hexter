class Color {
   ...
   */
  hsl() {
    // scale the R, G, B values to fill the [0,1] interval.
    const r = this.r / 255;
    const g = this.g / 255;
    const b = this.b / 255;

    return {
      hue: this.calculateHue(r, g, b),
      saturation: this.calculateSaturation(r, g, b),
      luminance: this.calculateLuminance(r, g, b),
    };
  }

  //#region hsl helper functions
  /**
   * @param {number} r red value scaled to [0, 1] bounary
   * @param {number} g green value scaled to [0, 1] bounary
   * @param {number} b blue value scaled to [0, 1] bounary
   * @returns {number} hue calculated from RGB
   */
  calculateHue(r, g, b) {
    ...
    return hue % 100 < 100 ? hue : hue * 100;
  }
   ...
   * @param {number} g green value scaled to [0, 1] bounary
   * @param {number} b blue value scaled to [0, 1] bounary
   * @returns {number} saturation calculated from RGB
   */
  calculateSaturation(r, g, b) {
    ...
    let result = avg <= 0.5 ? diff / sum : diff / (2 - diff);
   ...
   * @param {number} g green value scaled to [0, 1] bounary
   * @param {number} b blue value scaled to [0, 1] bounary
   * @returns {number} luminance calculated from RGB
   */
  ...
  multiply(matrix) {
    ...
    this.r = this.clamp(newR);
    this.g = this.clamp(newG);
    this.b = this.clamp(newB);
  }

  //#region css filter props formation
  hueRotate(angle = 0) {
    angle /= 180;
    angle *= Math.PI;
...
class Solver {
  ...
  loss(filters) {
    ...
    return (
      ...
      Math.abs(colorHSL.hue - this.targetHSL.hue) +
      Math.abs(colorHSL.saturation - this.targetHSL.saturation) +
      Math.abs(colorHSL.luminance - this.targetHSL.luminance)
    );
  }
...
export function newSolver(color) {
  return new Solver(color);
}
